...
object NotificationUtil {
    ...
    fun createChannel() {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            val manager = TardaMasNaoFalhaApplication.getInstance().getSystemService(
                Context.NOTIFICATION_SERVICE
            ) as NotificationManager

            val c = NotificationChannel("1", "tardamasnaofalha",
    ...
    fun create (id: Int, intent: Intent, titulo: String, texto: String) {
        ...
        with (NotificationManagerCompat.from(TardaMasNaoFalhaApplication.getInstance())) {
            val n = builder.build()
            notify(id, n)
        }
    }
